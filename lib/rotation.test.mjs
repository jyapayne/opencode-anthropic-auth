...
describe("HealthScoreTracker", () => {
  ...
  it("applies passive recovery over time", () => {
    ...
    try {
      ...
      const afterPenalty = tracker.getScore(0);
      expect(afterPenalty).toBe(50);

      // Advance time by 1 hour — recovery_rate_per_hour is 2, so expect +2
      vi.advanceTimersByTime(60 * 60 * 1000);
      const afterOneHour = tracker.getScore(0);
      expect(afterOneHour).toBe(52);

      // Advance another 5 hours — expect +10 more (capped by floor)
      vi.advanceTimersByTime(5 * 60 * 60 * 1000);
      const afterSixHours = tracker.getScore(0);
      expect(afterSixHours).toBe(62);
    } finally {
      vi.useRealTimers();
    }
  });
...
describe("TokenBucketTracker", () => {
  ...
  it("regenerates tokens over time", () => {
    ...
    // regeneration_rate_per_minute is 6, so after 1 minute expect +6
    vi.advanceTimersByTime(60 * 1000);
    expect(tracker.getTokens(0)).toBe(46);

    // After another minute, expect +6 more = 52, but capped at max_tokens (50)
    vi.advanceTimersByTime(60 * 1000);
    expect(tracker.getTokens(0)).toBe(50);
  });
});
